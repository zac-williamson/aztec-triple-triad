...
export const TUTORIAL_SCENES: TutorialScene[] = [
  // ── SCENE 0 — Introduction ─────────────────────────────────────────────
  {
    ...
    dialogue: [
      { text: 'Hello dear. A new soul wanders into my swamp.', pauseAfterMs: 1500 },
      { text: 'You want to play? Let\'s show you the rules.' },
      { text: 'Each of us holds five cards. Three public, two hidden.' },
      { text: 'We take turns placing cards on the board. Whoever owns the most cards wins.' },
    ],
    highlights: [
      { kind: 'PLAYER_HAND' },
  ...
  // ── SCENE 1 — Card Anatomy + Xochitl's first move ─────────────────────
  {
    ...
    dialogue: [
      { text: 'See those numbers? Each edge of a card has a rank.' },
      { text: 'When we place cards, the touching edges are compared. If yours is higher — my card flips to your colour.' },
      { text: 'I will go first. Watch.' },
    ],
    highlights: [{ kind: 'PLAYER_HAND' }],
    xochitlMoveAfter: { cardId: 1, row: 1, col: 1 },
  ...
  // ── SCENE 2 — Player places Vine Creeper above Swamp Sprite ───────────
  {
    ...
    dialogue: [
      { text: 'My card has only 1 on its top.' },
      { text: 'Capture it with your Blushy' },
    ],
    highlights: [
      { kind: 'HAND_CARD', cardId: 2 },
  ...
  // ── SCENE 3 — Capture reaction + Xochitl plays Reed Dancer ────────────
  {
    ...
    dialogue: [
      { text: 'Watch the score. Every card on the board counts.' },
    ],
    highlights: [{ kind: 'SCORE_DISPLAY' }],
    xochitlMoveAfter: { cardId: 3, row: 2, col: 2 },
  ...
  // ── SCENE 4 — Player places River Drake ───────────────────────────────
  {
    ...
    trigger: { kind: 'TURN_START' },
    dialogue: [
      { text: 'My turn. I can take your weak Blushy and trigger a chain capture.' },
      { text: 'But now my card card is weak on its top edge - see if you can take it out.' },
    ],
    highlights: [
      { kind: 'HAND_CARD', cardId: 10 },
  ...
  // ── SCENE 5 — Reaction + Xochitl plays Mud Golem ─────────────────────
  {
    ...
    trigger: { kind: 'ANY_CARD_PLACED' },
    dialogue: [
      { text: 'Adjacency works in all four directions.' },
    ],
    highlights: [{ kind: 'NONE' }],
    xochitlMoveAfter: { cardId: 11, row: 1, col: 0 },
  ...
  // ── SCENE 6 — Player places Marsh Hawk ────────────────────────────────
  {
    ...
    highlights: [
      { kind: 'HAND_CARD', cardId: 4 },
      { kind: 'BOARD_CELL', row: 2, col: 0 },
      { kind: 'SCORE_DISPLAY' },
    ],
    allowPlayerAction: true,
  },
  ...
  // ── SCENE 7 — Reaction + Xochitl plays Bog Witch ─────────────────────
  {
    ...
    trigger: { kind: 'ANY_CARD_PLACED' },
    dialogue: [
      { text: 'I\'m trapped - none of my cards can capture.' },
    ],
    highlights: [{ kind: 'NONE' }],
    xochitlMoveAfter: { cardId: 2, row: 0, col: 2 },
    xochitlMoveDelayMs: 1200,
    allowPlayerAction: false,
    autoAdvance: true,
  },
  ...
  // ── SCENE 9 — Reaction + Xochitl plays Swamp King (game ends) ────────
  {
    ...
    highlights: [{ kind: 'NONE' }],
    xochitlMoveAfter: { cardId: 4, row: 0, col: 0 },
