...
export interface ArenaBotConfig {
  ...
  /**
   * How long to hold a game open after the relay reports the opponent's socket
   * dropped. The relay's own reconnection window is 60s, so this is that plus
   * a margin — long enough that a blip is not a forfeit, short enough that one
   * departure does not take the only opponent in the arena offline for half an
   * hour.
   */
